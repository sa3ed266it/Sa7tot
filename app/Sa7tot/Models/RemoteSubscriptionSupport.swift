...
enum SubscriptionDisplayIdentity {
    ...
    static func serviceName(for subscription: RemoteSubscriptionBriefDTO) -> String {
        if let displayName = normalized(subscription.displayName) {
            return displayName
        }
        if let serviceID = subscription.serviceID,
           let service = SubscriptionServiceCatalog.service(forID: serviceID) {
            return service.displayName
        }
        if let serviceID = normalized(subscription.serviceID) {
            return serviceID
        }
        return AppLocalization.string("subscription.detail")
    }

    static func isAutoGeneratedServiceNote(_ note: String, subscription: RemoteSubscriptionBriefDTO) -> Bool {
        guard let trimmedNote = normalized(note) else { return false }
        let slugNote = SubscriptionCatalogNormalizer.serviceID(trimmedNote) ?? trimmedNote.lowercased()
        ...
        if let serviceID = subscription.serviceID,
           let service = SubscriptionServiceCatalog.service(forID: serviceID) {
            candidates.append(service.displayName)
            candidates.append(contentsOf: service.aliases)
        }

        for candidate in candidates {
            guard let candidate = normalized(candidate) else { continue }
            if candidate.lowercased() == trimmedNote.lowercased() {
                return true
            }
            if let slugCandidate = SubscriptionCatalogNormalizer.serviceID(candidate), slugCandidate == slugNote {
                return true
            }
        }
        return false
